...
int
funera_classic_dump_stack(WASMExecEnv *exec_env, struct WASMInterpFrame *frame)
{
...
int funera_classic_dump_memory(WASMMemoryInstance *memory) {
...
int funera_classic_dump_global(WASMModuleInstance *module, WASMGlobalInstance *globals, uint8* global_data) {
...
int funera_classic_dump_program_counter(
    WASMModuleInstance *module,
...
int funera_classic_dump(WASMExecEnv *exec_env,
         WASMModuleInstance *module,
         ...
         register uint8 *frame_ip)
{
    ...
    rc = funera_classic_dump_memory(memory);
    ...
    // dump globals
    clock_gettime(CLOCK_MONOTONIC, &ts1);
    rc = funera_classic_dump_global(module, globals, global_data);
    clock_gettime(CLOCK_MONOTONIC, &ts2);
    ...
    clock_gettime(CLOCK_MONOTONIC, &ts1);
    rc = funera_classic_dump_program_counter(module, cur_func, frame_ip);
    clock_gettime(CLOCK_MONOTONIC, &ts2);
    ...
    // dump stack
    clock_gettime(CLOCK_MONOTONIC, &ts1);
    rc = funera_classic_dump_stack(exec_env, frame);
    clock_gettime(CLOCK_MONOTONIC, &ts2);
